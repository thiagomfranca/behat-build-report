...
namespace Softbox\BehatBuildReport\Formatter;
...
use Softbox\BehatBuildReport\Classes\Feature;
use Softbox\BehatBuildReport\Classes\Scenario;
use Softbox\BehatBuildReport\Classes\Step;
use Softbox\BehatBuildReport\Classes\Suite;
use Softbox\BehatBuildReport\Printer\FileOutputPrinter;
use Softbox\BehatBuildReport\Renderer\BaseRenderer;
 ...
 */
class BehatReportFormatter implements Formatter {
   ...
   * @param $base_path
   */
    public function __construct($name, $build, $matrix, $filename, $base_path) {
        $this->name = $name;
        $this->build = $build;
        $this->matrix = $matrix;
        $this->renderer = new BaseRenderer('Behat2', $base_path);
  ...
  public function getSkippedSteps() {
    ...
    //      dump($event->getSpecificationIterators());exit;
        $this->timer->start();
       ...
       if (!is_dir($dsPath)) {
        ...
        file_put_contents($this->filename, $print, FILE_APPEND);
...
//      $this->dumpObj($event->getStep()->getArguments());exit;
    /** @var Step $step */
    $step = new Step();
